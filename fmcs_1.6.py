...
for i in np.arange(len(nodos_total)):
    if (nodos_total[i+1].node_type==0): nn+=1
    if (nodos_total[i+1].node_type==1): nr+=1
    if (nodos_total[i+1].node_type==2): nt+=1
    if (nodos_total[i+1].results[de][0]>0): nd+=1
for i in np.arange(len(lineas_total)):
    if (lineas_total[i+1].link_type==1): tt+=1
...
for niv in a:
    for co in cor.a_cut(niv):
        ...
        for j in np.arange(ns):
            ...
            for line in red_ini:
                if(i>=fi and i<fi+nd):
                    red_temp.write(dn[i-fi]+" "+str("%0.6f" % rl[j][i-fi])+"\n")
                else:
                    red_temp.write(line)
                i+=1
            red_temp.close()
